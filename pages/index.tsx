...
const ChatBubbleWrap = styled.div<{ whose: Chat["whose"] }>`
  ${TextSansStyle}
  svg {
    ${({ whose }) =>
      whose === "BRIDE"
...
const ChatBubble = ({ chat }: ChatBubbleProps) => {
  return (
    <ChatBubbleWrap whose={chat.whose}>
      {chat.whose === "BRIDE" ? <EmojiLookLeft /> : <EmojiLookRight />}
      <div>
        {chat.author}
        <br />
        <p>{chat.msg}</p>
